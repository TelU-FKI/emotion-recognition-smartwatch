...
def permute(results):
    ...
    (p, diff_means) = one_sample(svm_lift, stat='mean')
    print(('SVM\t\t{roc:.3f} ({roc_std:.3f})\t{f1:.3f} ({f1_std:.3f})\t{acc:.3f} ({acc_std:.3f})\t{diff:.3f}\t{p:.3f}'.format(
            roc=svm_roc.mean(), roc_std=svm_roc.std(),
            f1=svm_f1.mean(), f1_std=svm_f1.std(),
            acc=svm.mean(), acc_std=svm.std(),
            diff=diff_means, p=p)))
