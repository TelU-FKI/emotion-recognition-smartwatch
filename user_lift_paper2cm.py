    ...
    def process_condition(fnames, condition):
        ...
        for fname in fnames:
            ...
            for key, clf in models:
                scores = {'f1':[], 'acc':[], 'roc_auc':[], 'conf_matrix':[]}
                for i, (train, test) in enumerate(rskf.split(x_data, y_data)):
                    ...
                    if hasattr(clf, 'predict_proba'):
                        ...
                        _roc_auc = None
                    _conf_matrix = metrics.confusion_matrix(y_test, y_pred)
                    scores['f1'].append(_f1)
                    scores['acc'].append(_acc)
                    scores['conf_matrix'].append(_conf_matrix)
                ...
                results[key]['conf_matrix'].append(np.sum(scores['conf_matrix'], axis=0))  # Average confusion matrix
