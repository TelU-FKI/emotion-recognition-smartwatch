...
def permute(results):
    ...
    adaboost = np.array(results['adaboost']['acc'])
    gb = np.array(results['gb']['acc'])
    lgb = np.array(results['lgb']['acc'])

    baseline_f1 = np.array(results['baseline']['f1'])
    logit_f1 = np.array(results['logit']['f1'])
    rf_f1 = np.array(results['rf']['f1'])
    adaboost_f1 = np.array(results['adaboost']['f1'])
    gb_f1 = np.array(results['gb']['f1'])
    lgb_f1 = np.array(results['lgb']['f1'])

    baseline_roc = np.array(results['baseline']['roc_auc'])
    logit_roc = np.array(results['logit']['roc_auc'])
    rf_roc = np.array(results['rf']['roc_auc'])
    adaboost_roc = np.array(results['adaboost']['roc_auc'])
    gb_roc = np.array(results['gb']['roc_auc'])
    lgb_roc = np.array(results['lgb']['roc_auc'])

    logit_lift = logit - baseline
    rf_lift = rf - baseline
    adaboost_lift = adaboost - baseline
    gb_lift = gb - baseline
    lgb_lift = lgb - baseline
    ...
    (p, diff_means) = one_sample(adaboost_lift, stat='mean')
    ...
    (p, diff_means) = one_sample(gb_lift, stat='mean')
    ...
    (p, diff_means) = one_sample(lgb_lift, stat='mean')
    print(('LGB\t\t{roc:.3f} ({roc_std:.3f})\t{f1:.3f} ({f1_std:.3f})\t{acc:.3f} ({acc_std:.3f})\t{diff:.3f}\t{p:.3f}'.format(
            roc=lgb_roc.mean(), roc_std=lgb_roc.std(),
            f1=lgb_f1.mean(), f1_std=lgb_f1.std(),
            acc=lgb.mean(), acc_std=lgb.std(),
            diff=diff_means, p=p)))
